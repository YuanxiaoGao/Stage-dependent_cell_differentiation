...
for b_in in range(len(b_points)):  
	...
	fig.suptitle(r"$0 \leq \delta \leq 0.1$, $b=%s$, $c=%s$"%(b,round(c,2)),fontsize=14,y=1)
	...
	plt.legend(handles=legend_elements,ncol=6, loc='upper center', bbox_to_anchor=(0.5, 1.115),fontsize=11,frameon=False)
	...
	dp_ex=[np.array([[1,0,0],[1,0,0]]),np.array([[1,0,0],[1,0,0]]),np.array([[1,0,0],[1,0,0]]),
		np.array([[1,0,0],[1,0,0]]),np.array([[0,0,1],[0,0,1]])]
...
#		plt.title(r"$b=%s$, $c=%s$"%(bc_points[b],bc_points[c]),fontsize=12)

	plt.show()
	fig.savefig('./extrem_fg_r_grate_b%s_c%s_dup20.pdf'%(b,c), bbox_inches = 'tight')   # save figures
