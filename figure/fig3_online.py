...
b_points=[0.1,1,10,60]
c_points=[]
for x in b_points:
	c_points.append(4*x/(2+x))
#-------------------------------------------------------------------------------
'''read data '''
'''read data '''
grid_num=len(b_points)
result=[[[] for stra in range(4)] for b in range(5)]   # save the four categories
	...
	for stra_cluster in range(4): 
		for dup in range(num_dup): 
			for n in range(5,6):  
				...
				if os.path.exists('../data/data_fig3_dots_online/%s_%s_%s_%s.txt'%(dup,n,b,stra_cluster)):
					data=np.loadtxt('../data/data_fig3_dots_online/%s_%s_%s_%s.txt'%(dup,n,b,stra_cluster))						
...
for b_in in range(len(b_points)):  
	...
	fig.suptitle(r"$\delta=0.1$, $b=%s$, $c=%s$"%(b,round(c,2)),fontsize=14,y=1)
	...
	plt.legend(handles=legend_elements,ncol=5, loc='upper center', bbox_to_anchor=(0.5, 1.115),fontsize=10,frameon=False)
	...
	ax1.set_ylabel(r'Frequency of germ-like cell, $f_{g}^{(i)}$',fontsize=14, labelpad=0)
	...
	ax3.plot(2,extrem,marker="h",markersize=12,color="k",alpha=0.8 ,zorder=3)
...
#		"growthe rate lambda"
		...
		for run in range(num_dup):
			...
			ax1.scatter(size,fg_np[run] ,s=0.5,marker="o",label=label_list[stra_cluster+1],color=color_list[stra_cluster+1],alpha=alp_small)
		
			ax2.plot(size,r_np[run] , linestyle = '-',linewidth=0.3,color=color_list[stra_cluster+1],alpha=alp_small)	
			ax2.scatter(size,r_np[run] ,s=0.5,marker="o",label=label_list[stra_cluster+1],color=color_list[stra_cluster+1],alpha=alp_small)
		
			ax3.scatter(size[stra_cluster+1],grate_list[run] ,s=1,marker="o",label=label_list[stra_cluster+1],color=color_list[stra_cluster+1],
					alpha=1,zorder=5)
		"shades"
		...
		ax3.set_ylim(0.25,1.9)

	plt.show()
#	fig.savefig('./fig3_3online_extrem_fg_r_grate_b%s_c%s_dup20.pdf'%(b,c), bbox_inches = 'tight')   # save figures
